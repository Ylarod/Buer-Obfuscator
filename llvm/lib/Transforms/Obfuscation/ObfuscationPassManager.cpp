...
struct ObfuscationPassManager : public ModulePass {
  ...
  bool runOnModule(Module &M) override {
    ...
    add(llvm::createFlatteningPass(EnableIRFlattening || Options->EnableCFF,
                                   IPO, Options.get()));
    add(llvm::createIndirectBranchPass(EnableIndirectBr || Options->EnableIndirectBr, IPO, Options.get()));
    add(llvm::createIndirectCallPass(
        EnableIndirectCall || Options->EnableIndirectCall, IPO, Options.get()));
    add(llvm::createIndirectGlobalVariablePass(
        EnableIndirectGV || Options->EnableIndirectGV, IPO, Options.get()));
